...
def update_mobile(name, otp, new_mobile):
...
def order_confirmation_sms(name, new_mobile, order_id):
...
def cancel_order_sms(name, new_mobile, amount, order_id):
...
def shero_subscription_plan_activation(name, new_mobile):
...
def cancel_subscription_sms(name, new_mobile):
