...
from adminpanel.models import GuestUserData, ProductOrderData, ShopProduct, UserAddress, WebCity, WebCountry, WebState, UserCart, TokenAndSecretKey
...
import ast
 # country_name = "US"
    ...
    # shop_product = ShopProduct.objects.get(id=27)
    # shop_product.weight = 3
    # shop_product.units = "ounces"
    # shop_product.dimensions_unit = "inches"
    # shop_product.dimensions_length = 7
    # carrier_code = 'stamps_com' if carrier_name == 1 else 'ups_walleted'
    
    # state = WebState.objects.get(id=address.web_state_id)
    # city = WebCity.objects.get(id=address.web_city_id)
    # state_name = state.name
    # city_name = city.name
# try:
    # token_and_key = TokenAndSecretKey.objects.get(name="SHIP_STATION_TOKEN")
headers = {
    "Host": "ssapi.shipstation.com",
    "Authorization": settings.SHIP_STATION_TOKEN,
    # "Authorization": token_and_key.key,
...
def shipping_station(request):
    ...
    if user_cart.shop_product_list:
        ...
        product_list = ast.literal_eval(user_cart.shop_product_list)
...
def guest_user_shipping_station(request, cart_data):
    ...
    if guest_user_data:
        ...
        if cart_data.shop_product_list:
            # making list from the string
            product_list = ast.literal_eval(cart_data.shop_product_list)
            #  then list of str converted to int
            shop_product_list = list(map(int, product_list))
            shop_products = ShopProduct.objects.filter(id__in=shop_product_list)
            product_data = []
...
def create_order(product_order):
    ...
    for item in product_order_data:
        items.append({
            ...
            "unitPrice": float(item.quantity) * float(item.per_unit_price),
    ...
    payload = {
        ...
        "shipTo": {
            "name": product_order.user.name,
            "company": None,
            "street1": f'{ship_to.house_num} {ship_to.area_name}',
            "street2": None,
            "street3": None,
        ...
        "items": items,
        "amountPaid": product_order.total_amount,
        "taxAmount": product_order.tax,
        "shippingAmount": product_order.shipping_charge,
        "tagIds": []
    }
    data = json.dumps(payload)
...
def guest_user_create_order(guest_user_data):
    ...
    total = float(guest_user_data.total_amount) + shipping_charge + tax
    
    for item in guest_user_data.order_detail:
        items.append({
            "name": item.get('name', None),
            "quantity": item.get('quantity'),
            "unitPrice": float(item.get('amount')),
    ...
    payload = {
        ...
        "billTo": {
            ...
            "company": None,
            "street1": f"{ship_to.get('house_number')} {ship_to.get('area_name')}",
            "street2": None,
            "street3": None,
...
def delete_order(order_id):
    url = "https://ssapi.shipstation.com/orders/"+str(order_id)
    payload = {}
