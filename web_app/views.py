...
from .email import *
from .tax import tax_jar_tax_amount, guest_user_taxjar_tax_amount
...
from adminpanel.global_dict import interval_dict
...
def web_verify_mail(request, email):
    try:
        user_data = User.objects.get(email = email)
        name = user_data.name
        if User.objects.filter(email=email, is_verified=True):
            messages.success(request, "Already verified")
            return redirect('web_login')
        User.objects.filter(email=email).update(is_verified=True)
        messages.success(request, "Verification Successful")
        try:
            welcome_mail_for_signing_up(email, name)
        except:
            pass
        return redirect('web_login')
    except:
        ...
        try:
            ...
            if userobj:
                ...
                try:
                    ...
                    pass    
                otp_to_reset_password(otp, email)
                messages.success(request, "OTP Sent to your Email")
                return redirect('/otp/' + str(user_id))
        except:
    ...
    if request.method == "POST":
        ...
        if userobj:
            if not otp:
                ...
                return redirect('/otp/' + str(id))
            if userobj.otp == otp:
                return redirect('/resetpassword/' + str(id))
            else:
                messages.error(request, 'Wrong OTP')               
            return redirect('/otp/' + str(id))
    return render(request, 'otp.html', {'email':email, 'user_id': mail.id})
    ...
    try:
        ...
        if request.method == "POST":
            try:
                ...
                Notification.objects.create(sender = email, receiver = contact_us_details.email, recevied_at = time, message = notification_message, notification_type = 'INQUIRY')
                message = "Hi, " + "Megadolls" + "," + description
                subject, from_email, to = "User's Inquiry",email, contact_us_details.email
...
def create(request):
    ...
    return render(request, 'pages/create.html', {'imagine_products': products, 'imagine_product_images': product_images, 'image_sub_tables': image_sub_tables, 'dress_types': dress_types, 'colors': colors, 'site_url': settings.SITE_URL})
...
def play(request):
    ...
    colors = ProductColor.objects.filter(is_active=True)
    dress_types = DressType.objects.filter(is_active=True)
    return render(request, 'pages/play.html', {'products': products, 'product_images': product_images, 'image_sub_tables': image_sub_tables, 'colors': colors, 'dress_types': dress_types, 'site_url': settings.SITE_URL})
...
def shop_product_data(subcategory_id=None):
    ...
    return {'subcategories_for_filter': subcategories_for_filter, 'collections_for_filter': collections_for_filter, 'shop_products': shop_products, 'shop_product_images': shop_product_images, 'image_sub_tables': image_sub_tables, 'collections': collections, 'subcategories': subcategories}
...
def string_to_list(str_list):
    # making list from the string
    if str_list:
        product_list = ast.literal_eval(str_list)
        #  then list of str converted to int
        shop_product_list = list(map(int, product_list))
...
def add_shero_to_cart(request, slug):
    ...
    messages.success(request, "Shero Doll added in the cart")
    return redirect(request.META.get('HTTP_REFERER'))
...
def my_account_order(request):
    ...
    for order in product_order:
        time = order.order_at
        remaining_time = (datetime.now() - time).total_seconds() / 3600.0
        if remaining_time <= float(6):
...
def cancel_user_order(request):
    try:
        ...
        if remaining_time <= float(24):
            ...
            orders_data.canceled_at = current_time
            orders_data.canceled_by = "Customer"
            orders_data.save()
            delete_order(orders_data.order_id)
            try:
                cancel_order_sms(request.user.name, request.user.mobile, orders_data.total_amount, order_id)
            except:
                pass
    ...
    try:
        ...
        if request.method == "POST":
            ...
            if not UserSubscriptions.objects.filter(user_id=user_id).exists():
                customer = create_customer(request.user.email)
            ...
            subscription_payment = payment_for_subscription(line_items=line_items, success_url='success_shero_subscription/', cancel_url='my_account_subscription/', user_id = user_id, customer_id=customer_id)
...
def success_shero_subscription(request):
    ...
    if checkout_session.status == 'complete':
        ...
        try:
            ...
            pass
        try:
            shero_subscription_plan_activation(request.user.name, request.user.mobile)
            # subscription_payment_confirmations_sms(request.user.name, request.user.mobile)
        except:
            pass
    else:
...
def redeem_shero_subscription(request):
    ...
    if request.method == "POST":
        ...
        if request.user.otp != otp:
            ...
            return redirect('redeem_my_gift_card')
        elif not UserGiftCard.objects.filter(receiver_email=request.user.email).exists():
...
def cancel_shero_subscription(request):
    ...
    if user_subscription.exists():
        ...
        user_subscription.save()
        try:
            cancel_subscription_sms(request.user.name, request.user.mobile)
        except:
            pass
        try:
            send_mail_subscription_cancel(request.user.email, request.user.name)
        except:
            pass
...
def subscription(request):
    try:
        ...
        return render(request, 'pages/subscription.html', {"subscription_content_data": subscription_content_data, "shero_dolls": shero_dolls, "shero_dolls_image": shero_dolls_image, "subscription_plan": subscription_plan, "subscription_benefits": subscription_benefits, 'user_subscription': user_subscription})
    except:
        messages.error(request, "Something Went Wrong")
        return redirect('home')
...
def buy_shero_dolls(request, slug):
    try:
        ...
        if user_subscription_data.subscription_id:
            ...
            return render(request, 'pages/previous-shero.html', {"shero_dolls": shero_dolls, "shero_dolls_image": shero_dolls_image, 'site_url': settings.SITE_URL, 'cart_item': cart_item})
        else:
...
def payment(request):
    ...
    total_amount = request.session['order_data'].get('total_amount')
    ...
    return render(request, 'pages/payment.html', {'STRIPE_PUB_KEY': settings.STRIPE_PUBLIC_KEY, 'sub_total': sub_total, 'shipment_cost': shipment_cost, 'tax': tax,'total_amount': total_amount, 'wallet_amount': wallet_amount})
...
def profile_edit(request ):
    try:
        ...
        if request.method == "POST":
            ...
            country_code = request.POST.get("country_code")
            new_mobile_number = request.POST.get('mobile')
            new_mobile = f"{country_code}{new_mobile_number}"
            ...
            if new_mobile_number: 
                try:
                    if new_mobile != user_data.mobile:
                        ...
                        user_data.save()
                        try:
                            update_mobile(user_data.name, otp, new_mobile) # this function is in twilio.py and used to send messgaes to mobile Number
                            messages.success(request, "Verification OTP Sent to Your New Number")
                            return redirect('mobile_update', new_mobile) 
                        except:
...
def mobile_update(request, new_mobile):
    id = request.user.id
    user_data = User.objects.get(id=id)
    otp = request.POST.get('otp')
    if otp:
        if otp == user_data.otp:
            ...
            messages.error(request, "Wrong OTP")
            return render(request, 'pages/profile/mobile_edit.html', {"new_mobile": new_mobile})
    return render(request, 'pages/profile/mobile_edit.html', {"new_mobile": new_mobile})

# to resend otp to mobile 
def resend_otp_mobile(request, new_mobile):
    id = request.user.id
    user_data = User.objects.get(id=id)
    otp = random.randint(100000, 999999)
    user_data.otp = otp
    user_data.save()
    try:
        update_mobile(user_data.name, otp, new_mobile) # this function is in twilio.py and used to send messgaes to mobile 
        messages.success(request, "Verification OTP Sent to Your New Number")
        return redirect('mobile_update', new_mobile) 
    except:
        messages.error(request, "Message Not Sent")      
        return redirect ('profile_edit', new_mobile)
# User Profile End
...
def create_checkout_session(request):
    if request.method == 'POST':
        ...
        amount_data = float(request.session['order_data'].get('total_amount'))
        remaining_pay_amount = amount_data
        ...
        if is_walled_selected:
            ...
            if wallet_balance >= amount_data:
                request.session['remaining_wallet_balance'] = wallet_balance - amount_data
                request.session.modified = True
                return JsonResponse({'success': True, 'is_wallet': True})
            else:
                remaining_pay_amount = amount_data - wallet_balance
    ...
    # try:
        ...
        if UserCart.objects.filter(user_id=user_id).exists():
            ...
            if further_proceed_status:
                ...
                for product in product_data:
                    product.quantity -= product_dict.get(product.id)
                    product.save()
                ...
                if has_remaining_pay_amount or has_remaining_wallet_balance:
                    if has_remaining_pay_amount:
                        amount_to_pay = request.session['remaining_pay_amount']
                        ...
                        amount_dict['wallet'] = product_order.paid_by_wallet = total_amount
                        del request.session['remaining_wallet_balance']
                    ...
                    amount_dict['stripe'] = product_order.paid_by_stripe = total_amount
                ...
                for data in product_data:
                    ...
                    try:
                        send_mail_order_place(request.user.email, product_order.order_id, product_order.order_at, amount_dict, request.user.is_authenticated, product_order)
                    except:
                        pass
                    try:
                        order_confirmation_sms(request.user.name, request.user.mobile, order_id)
                    except:
                        pass

                    user_and_order_backup = UserAndOrderBackup()
                    user_and_order_backup.user_name = request.user.name
                    user_and_order_backup.email = request.user.email
...
@login_required(login_url='web_login')
@csrf_exempt
def tax_amount(request):
    user_id = request.user.id
    shipping_cost = request.POST.get('shipping_cost')
    address_id = request.POST.get('shippingAddress')
    user_address = UserAddress.objects.get(id=int(address_id), user_id=user_id)
    user_cart = UserCart.objects.get(user_id=user_id)
    shop_product_list = string_to_list(user_cart.shop_product_list)
    product_dict = dict(Counter(shop_product_list))
    ...
    tax = tax_jar_tax_amount(data)
    
    return JsonResponse({'tax_amount': tax}, safe=False)

@csrf_exempt
def guest_user_tax_amount(request):
    shipping_cost = request.POST.get('shipping_cost')
    ...
    if guest_user_data:
        ...
        data = {
            ...
            'shipping_cost': shipping_cost,
            'product_detail': guest_user_data.order_detail,
        }

        tax = guest_user_taxjar_tax_amount(data)
        guest_user_data.tax = tax
        guest_user_data.shipping_charge = shipping_cost
        guest_user_data.save()
        return JsonResponse({'status': True, 'tax_amount': tax}, safe=False)
    return JsonResponse({'status': False, 'tax_amount': 0}, safe=False)
...
def save_order_data_to_session(request):
    request.session['order_data'] = {
        ...
        'carrier_service_name': request.POST.get('carrier_service_name'),
...
def guest_user_save_order_data_to_session(request):
    ...
    if guest_user_data:
        ...
        sub_total = float(guest_user_data.total_amount)
...
def create_gift_card(request):
    ...
    if request.method=='POST':
        ...
        receiver_email = data_dict.get('receiver_email')
        re_enter_receiver_email = data_dict.get('re_enter_receiver_email')
...
def create_shero_gift_card(request):
    ...
    if request.method=='POST':
        ...
        if not (receiver_email and not receiver_email.isspace() and message and not message.isspace()):
            ...
            try:
                checkout_session = gift_card_payment(line_item, success_url, cancel_url)
...
def success_payment_for_gift_card(request):
    ...
    if gift_code:
        ...
        sender_email_id = user_gift_card.sender_email
        if User.objects.filter(email = sender_email_id).exists():
            sender_email = User.objects.get(email = sender_email_id).email
        else:
            sender_email = sender_email_id
        ...
        stripe.api_key = settings.STRIPE_SECRET_KEY
        intent = stripe.PaymentIntent.retrieve(user_gift_card.payment_intent_id)
        ...
        if intent.status == 'succeeded':
            ...
            user_gift_card.amount = int(intent.amount/100)
            user_gift_card.save()
            
            receiver_email = user_gift_card.receiver_email
            ...
            if user_gift_card.gift_card_type == 'GIFT_CARD':
                ...
                # making img tag to send banner image on mail
                ...
                try:
                    ...
                    send_mail_giftcard_sender(receiver_email, sender_email, intent.charges.data[0].receipt_url, user_gift_card)
                except: 
                    pass
            ...
            try:
                ...
                send_mail_shero_subscription_gift_sender(user_gift_card.subscription_plan.plan_type, receiver_email, sender_email, intent.charges.data[0].receipt_url)
            except Exception as err:
                pass
...
def redeem_my_gift_card(request):
    data = UserGiftCard.objects.filter(Q(status='succeeded'), Q(receiver_email=request.user.email)).order_by('-id')
...
def send_referral_email(request):
    ...
    name = user.name
    email_to = request.POST.get('referral_email')
...
def testing(request):
    session = stripe.checkout.Session.create(
        success_url="http://127.0.0.1:8000/testing-success/",
        ...
        line_items=[
            {
                "quantity": 1,
                "price": "price_1Lutc9EoxvNu9SZplxLzD7N0"
                
            },
            {
                "quantity": 2,
                "price": "price_1LutcAEoxvNu9SZpJAmvOwj4"
...
def unsubscribe_to_blog(request, email):
    messages.success(request, "Unsubscribed")
    BlogSubscriber.objects.get(email=email).delete()
    return redirect("home")


def unsubscribe_to_newsletter(request, email):
    messages.success(request, "Unsubscribed")
    Newsletters.objects.get(email=email).delete()
    return redirect("home")    
...
def guest_user_billing(request):
    try:
        ...
        data = {
            # 'carriers': carriers, 
            'sub_total': cart_sub_total(request),
            'guest_user_address': GuestUserData.objects.filter(order_id=order_id).first(),
            'countries': WebCountry.objects.all()
        }
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if guest_user_data:
                ...
                total_amount += float(guest_user_data.tax) + float(guest_user_data.shipping_charge)
                domain = settings.SITE_URL
                stripe.api_key = settings.STRIPE_SECRET_KEY
                checkout_session = stripe.checkout.Session.create(
                    payment_method_types=["card"],
...
def guest_user_stripe_success(request):
    try:
        ...
        if request.session.has_key('guest_user') and request.session.has_key('guest_user_data'):
            ...
            sub_total = float(guest_user_data.total_amount)
            total_amount = sub_total + shipment_cost + tax
...
def download_count_increment(request):
    try:
        if request.user.download_count < 3:
            request.user.increase_download_count()
            return JsonResponse({"success": True, "message": f"You can download up to 3 times and you've downloaded {request.user.download_count} times"})
        else:
            return JsonResponse({"success": False, "message": "Maximum daily limit is 3 times"})
    except:
        return JsonResponse({'success': False, "message": "something went wrong"})
...
def render_sloper_landing_page(request):
    ...
    return render(request, 'pages/sloper-landing-page.html', {'banner_image': banner_image, 'how_it_work': how_it_work, 'how_it_work_section': how_it_work_sections, 'testimonial': testimonial, 'testimonial_sections': testimonial_sections})