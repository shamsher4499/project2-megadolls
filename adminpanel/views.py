...
def dashboard(request):
    ...
    dolls_out_of_stock = ShopProduct.objects.filter(quantity=0, subcategory__id=1).count()
    doll_sets_out_of_stock = ShopProduct.objects.filter(quantity=0, subcategory__id=2).count()
    dress_sets_out_of_stock = ShopProduct.objects.filter(quantity=0, subcategory__id=3).count()
    # total_out_of_imagine_products = ImagineProduct.objects.filter(quantity=0).count()

    total_shop_products_count = ShopProduct.objects.all().count()
    total_imagine_products_count = ImagineProduct.objects.all().count()

    total_products_count = total_shop_products_count + total_imagine_products_count

    total_dolls_count = ShopProduct.objects.filter(subcategory__id=1).count()
    total_doll_sets_count = ShopProduct.objects.filter(subcategory__id=2).count()
    total_dress_sets_count = ShopProduct.objects.filter(subcategory__id=3).count()
    total_create_count = ImagineProduct.objects.filter(subcategory__id=4).count()
    total_play_count = ImagineProduct.objects.filter(subcategory__id=5).count()
    ...
    total_revenue = total_order_revenue + total_subscription_revenue
...
def view_user_detail(request, user_id):
    ...
    return render(
        request,
        "user_management/view_user.html",
        {
            "user": user,
            "m": permission(request),
            "module_list": helper(request),
...
def add_super_admin(request):
    ...
    if request.method == "POST":
        try:
            ...
            ###### mail send start ###########
            try:
                email_template = EmailTemplate.objects.get(id=mail_credentail)
            except:
                messages.error(request, "Email Template Not found")
                return redirect("view_super_admins")

            subject, from_email, to = email_template.subject, settings.EMAIL_HOST_USER, email
            html_content = email_template.content
            html_content_formatted = html_content.format(name=name, password=password)
            msg = EmailMultiAlternatives(
                subject, html_content_formatted, from_email, [to]
            )
            msg.attach_alternative(html_content_formatted, "text/html")
            msg.send()
            ###### mail send end ###########
...
def add_newsletter(request):
    ...
    if request.method == "POST":
        email = request.POST.get("email")
        news = Newsletters.objects.create(email=email)
        news.save()
...
# when order cancelled this function is used to refund the amount which paid from wallet
def refund_to_wallet(request, id):
    order = ProductOrder.objects.filter(id=id)
    if order:
        order = order.last()
        if request.method == 'POST':
            ...
            if user:
                ...
                order.save()
                admin_refund_amount_to_wallet_order_mail(user.email, order.order_id, datetime.now())
...
def add_subscription_plan(request):
    try:
        ...
        if request.method == "POST":
            ...
            subscription = SubscriptionPlan.objects.create(plan_type=subscription_type, original_price=original_price, offer_price=offer_price, subscription_price_id=subscription_price_obj.id, plan_id=plan.id)
...
def view_referral_details(request, id):
    ...
    if request.method == "POST":
        ...
        dolls_got_count = user_data.dolls_got_count
        user_data.dolls_got_count = dolls_got_count + dolls_sent_count
        dolls_to_get_count = user_data.dolls_to_get_count
        user_data.dolls_to_get_count = dolls_to_get_count - dolls_sent_count
        user_data.save()
...
# To view details of sloper template
...
def sloper_multicolor_icon(request):
    try:
        multicolor_icon_data = SloperMulticolorIconSVG.objects.all()
...
def add_sloper_multicolor_icon(request):
    try:
        if request.method == "POST":
            ...
            status = request.POST.get("status")
            image = request.FILES.get("image")
            SloperMulticolorIconSVG.objects.create(name = name, status = status, image = image)
            messages.success(request, "Added")
            return redirect("sloper_multicolor_icon")
        multicolor_icon_data = SloperMulticolorIconSVG.objects.filter(status = True)
        return render(request, "sloper_tool_management/svg/add.html", {'multicolor_icon_data': multicolor_icon_data, "m": permission(request),
            "module_list": helper(request),
            "m_check": per(request)})
    except:
        messages.success(request, "Something Went Wrong ")
...
def edit_sloper_landing_page(request):
    banner_images = SloperLandingPageBannerImage.objects.all()
    how_it_works = SloperLandingPageHowItWork.objects.all()
    how_it_work_section = SloperLandingPageHowItWorkSection.objects.all()
    testimonial = SloperLandingPageTestimonial.objects.all()
    testimonial_section = SloperLandingPageTestimonialSection.objects.all()
    if request.method == "POST":
        pass
    return render(request, 'sloper-landing-page-management/edit.html', {'banner_images': banner_images, 'how_it_works': how_it_works, 'how_it_works_section': how_it_work_section, 'testimonial': testimonial, 'testimonial_section': testimonial_section})
